...
- `-v, --verbose`: 詳細な情報を表示
- `--parse-file-name`: ファイル名からトラック番号とタイトルを抽出して更新する（ファイル名の形式: `数字 タイトル.mp3`、`数字-タイトル.mp3`、または `数字_タイトル.mp3`）
- `-h, --help`: ヘルプを表示
- `--version`: バージョン情報を表示
...
この例では、ファイル名が以下のいずれかの形式（数字 + 区切り文字 + タイトル + .mp3）の場合、ファイル名からトラック番号とタイトルを自動抽出してタグに設定します：
- `01 曲名.mp3`（スペース区切り）
- `01-曲名.mp3`（ハイフン区切り）
- `01_曲名.mp3`（アンダースコア区切り）

ファイル名がこのパターンに一致しない場合は、そのファイルは一切変更されません。
...
- **`--parse-file-name`オプションについて**: 
  - ファイル名からトラック番号とタイトルを抽出する際、ファイル名は以下のいずれかの形式である必要があります：
    - `数字 タイトル.mp3`（スペース区切り、例: `01 曲名.mp3`）
    - `数字-タイトル.mp3`（ハイフン区切り、例: `01-曲名.mp3`）
    - `数字_タイトル.mp3`（アンダースコア区切り、例: `01_曲名.mp3`）
  - パターンの詳細：
    - 数字は1つ以上の連続した数字（例: `1`、`01`、`123`）
    - 区切り文字はスペース、ハイフン（`-`）、またはアンダースコア（`_`）のいずれか1文字
    - タイトル部分は1文字以上の任意の文字列
    - ファイル名は必ず `.mp3` で終わる必要があります
